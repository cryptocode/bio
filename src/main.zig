...
pub fn main(init: std.process.Init) !void {
    var interpreter = try Interpreter.init(init.io);
    ...
    // Load the standard library
    {
        ...
        const cwd_absolute = try std.process.currentPathAlloc(init.io, allocator);
...
test "Evaluate standard library and test.lisp" {
    var interpreter = try Interpreter.init(std.testing.io);
    defer interpreter.deinit();
