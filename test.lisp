...
; We scope everything in a lambda so we can evaluate this file multiple times in a REPL session.
...
((lambda ()
    ...
    ; Test the Math type
    (var math (Math))
    (assert (= 2 (math (floor 2.97))))
    (assert (= 5 (math (max 3 5 2 1))))
    (assert (= nil (math (max))))
    ...
    ; Test the String type, using both a str variable and (String) directly.
    (var str (String))
    (assert (str (lowercase? 'abc)))
    (assert (str (lowercase? (str (lowercase 'ABC)))))
    (assert ((String)(uppercase? 'ABC)))
    (assert ((String)(uppercase? (str (uppercase 'abc)))))

    (assert (= 2 (indexof nums 3)))
    (assert (= nil (indexof nums 100)))

    (assert (= 2 (std-list-min-item '(5 3 9 2 4 2 5 6))))
    (assert (= 9 (std-list-max-item '(5 3 9 2 4 9 5 6))))
    ...
    (assert (= 1 (std-math-pow 2 0)))
    (assert (= 2 (std-math-pow 2 1)))
    (assert (= 4 (std-math-pow 2 2)))
    ...
    ; Quoted numbers are numbers when evaluated, while double quotes are always symbols
    (assert (number? '5))
    (assert (symbol? "5"))
    ...
    ; Containment
    (var containment-map (std-hashmap-new ("1" 2) (3 nil)))
    ...
    ; Hashmap
    (var mymap (std-hashmap-new ('1 2) (3 4)))
    (assert (hashmap? mymap))
    (assert (= (len mymap) 2))
    (std-hashmap-put mymap 5 6)
    (std-hashmap-put mymap 7 "Initial entry")
    (var initial-entry (std-hashmap-put mymap 7 "Another entry"))
    (assert (= initial-entry "Initial entry"))
    (assert (= (len mymap) 4))
    (assert (= (std-hashmap-get mymap 7) "Another entry"))
    (std-hashmap-remove mymap 7)
    (assert (= (std-hashmap-get mymap 7) nil))
    (assert (= (len mymap) 3))

    (var keys '())
    (var vals '())
    (std-hashmap-iterate mymap (λ (k v)
        (item-append! keys k)
        (item-append! vals v)))
    ...
    (var count-removed (std-hashmap-clear mymap))
    (assert (= count-removed 3))
    (assert (= (len mymap) 0))

    ; k -> '()
    (var hmlist (std-hashmap-new))
    (std-hashmap-append! hmlist 'a 1)
    (std-hashmap-append! hmlist 'a 2)
    (assert (= (std-hashmap-get hmlist 'a) '(1 2)))

    ; The 'a entry exists, so the lambda is called, which updates the list
    (std-hashmap-put-or-apply hmlist 'a 3 (λ (list)
        (assert (= list '(1 2)))
        (item-append! list 3)))
    (assert (= (std-hashmap-get hmlist 'a) '(1 2 3)))
    (assert (= (std-hashmap-maybe-put hmlist 'a '(5 5 5)) '(1 2 3)))
    (assert (= (std-hashmap-maybe-put hmlist 'b '(5 5 5)) '(5 5 5)))
    ...
    ; Test type instance, and then pass the instance to set! as the environment argument
    ...
    ; Compare against the success branch
    (assert (= 3 (try (math.safe-div 6 2) #value)))
    ; Stringify error
    (assert (= "Division by zero" (try (math.safe-div 6 0) #value (string #!))))

    ; Test that quoted lists create fresh shallow copies.
    (var quoted-list-1 '(1 2 3))
    (var quoted-list-2 '(1 2 3))
    (assert (= quoted-list-1 quoted-list-2))
    (assert (not (^= quoted-list-1 quoted-list-2)))

    (fun local-list ()
        (var local '(1 2 3)))
    (var local-list-1 (local-list))
    (var local-list-2 (local-list))
    (assert (= local-list-1 local-list-2))
    (assert (not (^= local-list-1 local-list-2)))

    ; Test that (list 1 2 3) always create fresh shallow copies. This is guaranteed by the Bio specification.
    (var fresh-list-1 (list 1 2 3))
    (var fresh-list-2 (list 1 2 3))
    (assert (= fresh-list-1 fresh-list-2))
    (assert (not (^= fresh-list-1 fresh-list-2)))
    ...
    (var seq (Sequence))
    (assert (= '(a 1 b 2 c 3 d nil) (seq (zip '(a b c d) '(1 2 3)))))
